{{-- Do not include or render this file. It exists only so Tailwind's scanner
     detects dynamic class names used by the PageHero gradient. --}}
<div class="
    from-emerald-600 from-emerald-700 from-emerald-800
    from-teal-600 from-teal-700 from-teal-800
    from-sky-600 from-sky-700 from-sky-800
    from-indigo-600 from-indigo-700 from-indigo-800
    from-purple-600 from-purple-700 from-purple-800
    from-rose-600 from-rose-700 from-rose-800
    from-amber-500 from-amber-600 from-amber-700
    from-orange-600 from-orange-700
    from-slate-700 from-slate-800 from-slate-900

    via-emerald-600 via-emerald-700 via-emerald-800
    via-teal-600 via-teal-700 via-teal-800
    via-sky-600 via-sky-700 via-sky-800
    via-indigo-600 via-indigo-700 via-indigo-800
    via-purple-600 via-purple-700 via-purple-800
    via-rose-600 via-rose-700 via-rose-800
    via-amber-500 via-amber-600 via-amber-700
    via-orange-600 via-orange-700
    via-slate-700 via-slate-800 via-slate-900

    to-emerald-600 to-emerald-700 to-emerald-800
    to-teal-600 to-teal-700 to-teal-800
    to-sky-600 to-sky-700 to-sky-800
    to-indigo-600 to-indigo-700 to-indigo-800
    to-purple-600 to-purple-700 to-purple-800
    to-rose-600 to-rose-700 to-rose-800
    to-amber-500 to-amber-600 to-amber-700
    to-orange-600 to-orange-700
    to-slate-700 to-slate-800 to-slate-900
"></div>

{{-- Documentation: dynamic content from PageHero / GeneralSettings is server-rendered. --}}

{{-- Documentation for adding a new skin:
     1. Create folder resources/views/skins/{nama}/
     2. Create 4 partials: navbar.blade.php, footer.blade.php, page-hero.blade.php, page-frame.blade.php
        - navbar receives: $settings, $transparent
        - footer receives: $settings
        - page-hero receives: $key, $title, $subtitle, $breadcrumbs, $icon
        - page-frame receives: $padded, $frameSlot, $frameAttributes
     3. Add the new skin option in app/Filament/Pages/ManageGeneral.php (Select active_skin options).
     4. Run `npm run build` if you use Tailwind classes that are not yet referenced elsewhere.
--}}
